<template>
  <div>
    <div class="item">
      ...
      <button @click="complete" class="check" title="complete">
        <font-awesome-icon icon="check" />
      </button>
      <button @click="openEditModal" class="pen" title="edit">
        <font-awesome-icon icon="pen-to-square" />
      </button>
      <button @click="cancel" class="xmark" title="cancel">
...
export default {
  ...
  methods: {
    ...mapActions(['deleteItem', 'updateItem', 'completeItem', 'cancelItem']),
    async updateCheck() {
      try {
    ...
    async complete() {
      try {
        await this.completeItem(this.item.id);
        this.$emit('itemChanged');
      } catch (error) {
        console.error(error);
      }
    ...
    async cancel() {
      try {
        await this.cancelItem(this.item.id);
        this.$emit('itemChanged');
      } catch (error) {
        console.error(error);
      }
